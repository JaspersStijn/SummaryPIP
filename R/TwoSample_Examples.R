...
for(beta11 in c(0,-1,-4)){
  ...
  for( sampsize in c(20,40,60)){
    ...
    boxplot(output[,c("pip_cond1","pip_cond2","emp_cond","pip_exp","emp_exp" ,"pip_LOO","pip_SS","pip_CV5")],main=paste(paste0("Sample size: ",sampsize),paste0("Beta1: ",beta11),sep= "\n"))
...
Gaussian_out$pval_Ind=replace(Gaussian_out$pval_Ind,Gaussian_out$pval_Ind==FALSE,"pval>0.5")
Gaussian_out$pval_Ind=replace(Gaussian_out$pval_Ind,Gaussian_out$pval_Ind==TRUE,"pval<0.5")


Gaussian_out$EffectSize = EffectSize
Gaussian_out$SampleSize=Samplesize

Gaussian_out$SampleSize= factor(Gaussian_out$SampleSize, levels=c('Sample Size: 40','Sample Size: 100','Sample Size: 400'))
...
ggplot() + geom_point(data = subset(Gaussian_out,EffectSize=="Effect Size: 0"), aes(y = PIP, x = MSE_diff, color = pval_Ind)) + #geom_hline(yintercept = 0.5,linetype="dashed")+ geom_vline(xintercept = 0.05,linetype="dashed")+
  facet_wrap(~ EffectSize + SampleSize, scales = "free")
